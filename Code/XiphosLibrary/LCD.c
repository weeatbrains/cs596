...
/*! Prints up to maxChars of a null-terminated string from SRAM to the LCD starting at the
...
{
	u08 i = 0;
	//keep printing until we encounter a null terminator or have printed maxChars.
	while (string[i] != 0 && i < maxChars)
	{
		printChar(string[i++]);
	}
}
