...
void motorInit()
{
	//Set up Timer 1 in 8-bit fast PWM mode with the prescaler at I/O clock / 8,
	//with the PWM in normal polarity (meaning motor on or brake on is logic 1).
	//PWM frequency = F_CPU/prescaler/(1+TOP) = 16E6/8/(1+0xFF) = 7812.5 Hz,
	//which is less than the 10kHz maximum allowed by the VNH3SP30 H-bridge chip.
	TCCR1A = _BV(WGM10);
	TCCR1B = _BV(CS11) | _BV(CS10) | _BV(WGM12);
		...
		//Configure PWM as an output
		sbi(DDRB, DDB5);

		//Set OC1A (output compare unit 1A) to use normal polarity for PWM
		TCCR1A |= _BV(COM1A1);
		...
		//Set OC1B (output compare unit 1B) to use normal polarity for PWM
		TCCR1A |= _BV(COM1B1);
		//Set the duty cycle to zero
		OCR1B = 0;
	#endif
